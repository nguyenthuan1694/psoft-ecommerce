...
@section('content')
    ...
    <div class="container-fluid px-xxl-65 px-xl-20">
        ...
        <div class="hk-pg-header">
            <h4 class="hk-pg-title"><span class="pg-title-icon"><span class="feather-icon"><i data-feather="external-link"></i></span></span>List of trashed News</h4>
        </div>
        <!-- /Title -->
                                ...
                                <table class="table table-striped table-bordered">
                                    ...
                                    @foreach($news as $index => $items)
                                        <tr>
                                            <td>{{ $items->id }}</td>
                                            <td>{{ $items->title }}</td>
                                            <td><img src="{{ $items->thumbnail }}" class="img-fluid img-thumbnail" height="100" width="100" alt="img"></td>
                                            ...
                                            <td>
                                                ...
                                                <form name="restore_{{ $index }}" action="{{ route('news.restore', ['id' => $items->id]) }}" method="post">
                                                    @csrf
                                                    @method('PUT')
                                                </form>
                                                @include('backend.includes.modal_delete_confirm', ['index' => $index, 'item' => $items, 'type' => 'news'])
                                            </td>
                                        </tr>
                                    @endforeach
                                    @if(!count($news))
