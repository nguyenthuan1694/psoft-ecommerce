                        ...
                        <div class="col-sm">
                            <div class="table-wrap table-responsive">
                                <table class="table table-striped table-bordered table-sm" id="dtBasicExample">
                                    ...
                                    <tbody>
                                    @foreach($receipts as $index => $items)
                                        <tr>
                                            <td>{{ $items->id }}</td>
                                            ...
                                            <td>{{ $items->phone }}</td>
                                            <td>{{ $items->email }}</td>
                                            <td>{{ $items->fall_day }}</td>
                                            <td>{{ number_format($items->proceeds, 0) }}</td>
                                            <td>{{ number_format($items->amount_owed, 0) }}</td>
                                            <td>
                                                <a href="{{ route('receipt.edit', ['receipt' => $items->id]) }}" class="mr-25" data-toggle="tooltip" data-placement="top" title="Edit"> <i class="icon-pencil"></i> </a>
                                                ...
                                                <form name="restore_{{ $index }}" action="{{ route('receipt.restore', ['id' => $items->id]) }}" method="post">
                                                    @csrf
                                                    @method('PUT')
                                                </form>
                                                @include('includes.modal_delete_confirm', ['index' => $index, 'item' => $items, 'type' => 'receipt'])
                                            </td>
                                        </tr>
                                    @endforeach
                                    @if(!count($receipts))
